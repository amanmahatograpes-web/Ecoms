...
// const PAGE_SIZE = 10; // show 10 products per page
...
//       setError('Failed to fetch products. Please check backend or network.');
//       setProducts([]);
//     } finally {
//       setLoading(false);
//     }
...
//       case 'out_of_stock': return <Package className="h-4 w-4 text-red-500" />;
...
//       <p className="text-gray-600 mb-6">Manage your product inventory and stock levels</p>
...
//             className="w-full pl-10 pr-4 py-2 border rounded-lg border-gray-300 focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
//           />
//         </div>
//         <select
//           value={filterStatus}
...
//               <tr>
//                 {['Product', 'SKU', 'Category', 'Price', 'Quantity', 'Status', 'Actions'].map(col => (
...
//                         src={p.mainImage}
//                         alt={p.title || ''}
//                         className="h-10 w-10 object-cover rounded-lg mr-3 cursor-pointer"
...
//                   <td className="px-6 py-4 text-sm text-gray-900">${parseFloat(p.price || 0).toFixed(2)}</td>
//                   <td className="px-6 py-4">
//                     <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
//                       p.quantity === 0 ? 'bg-red-100 text-red-800' : p.quantity < 10 ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'
//                     }`}>{p.quantity ?? 0}</span>
//                   </td>
//                   <td className="px-6 py-4 flex items-center">
//                     {getStatusIcon(p.status)}
//                     <span className={`ml-2 px-2 py-1 text-xs rounded-full ${getStatusColor(p.status)}`}>{(p.status || '').replace('_',' ')}</span>
//                   </td>
//                   <td className="px-6 py-4 flex space-x-2">
//                     <button onClick={()=>setEditProduct(p)} className="p-1 text-green-600 hover:bg-green-50 rounded"><Edit size={16} /></button>
...
//         >
//           <img src={previewImage} alt="Preview" className="max-h-[80%] max-w-[80%] rounded-lg shadow-lg" />
//         </div>
//       )}
...
//           className="w-full bg-gray-200 mt-2 py-2 rounded-lg"
//         >
//           Cancel
//         </button>
//       </div>
...
const Inventory = ({ refreshTrigger }) => {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case 'active': return <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5 text-green-500" />;
      case 'inactive': return <AlertTriangle className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-500" />;
      case 'out_of_stock': return <Package className="w-4 h-4 sm:w-5 sm:h-5 text-red-500" />;
      default: return <Package className="w-4 h-4 sm:w-5 sm:h-5 text-gray-500" />;
    }
  };
  ...
  const exportToCSV = () => {
    const csvContent = [
      ['Title', 'SKU', 'Brand', 'Category', 'Price', 'Quantity', 'Status'],
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-6 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-2">Error Loading Inventory</h2>
    ...
    <div className="min-h-screen bg-gray-50 p-3 sm:p-4 md:p-5 lg:p-6 xl:p-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-0">
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 sm:px-4 py-2 sm:py-2.5 bg-orange-600 text-white rounded-lg hover:bg-orange-700 text-sm sm:text-base font-medium transition-colors"
            >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4 mb-4 sm:mb-6 lg:mb-8">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-3 sm:p-4">
          ...
          <div className="text-xl sm:text-2xl md:text-3xl font-bold text-green-600">{stats.active}</div>
        </div>
        ...
        <div className="col-span-2 sm:col-span-1 lg:col-span-1 bg-white border border-gray-200 rounded-xl p-3 sm:p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs sm:text-sm font-medium text-gray-700">Total Value</span>
            <DollarSign className="w-4 h-4 sm:w-5 sm:h-5 text-purple-500" />
          </div>
          <div className="text-xl sm:text-2xl md:text-3xl font-bold text-purple-600">${stats.totalValue.toLocaleString()}</div>
        </div>
      </div>
          ...
          <div className="hidden lg:flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <select
                value={filterStatus}
                onChange={e => setFilterStatus(e.target.value)}
                className="border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 text-sm"
              >
                <option value="all">All Status</option>
                <option value="active">Active</option>
                <option value="inactive">Inactive</option>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {currentProducts.map(p => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 sm:px-6 py-4">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${getStockColor(p.quantity)}`}>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className={`px-2 py-0.5 text-xs rounded-full border ${getStatusColor(p.status)}`}>
                            {getStatusText(p.status)}
                          </span>
                        </div>
              ...
              <div className="p-4 sm:p-5">
                ...
                <div className="space-y-2 text-xs sm:text-sm text-gray-600 mb-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className={`px-2 py-1 text-xs rounded-full ${getStockColor(p.quantity)}`}>
                      {p.quantity} units
                    </div>
                  </div>
                </div>
